...
public class GroupFileStorage {
	...
	public void saveGroupToCSV(Group gr) {
		...
		try (PrintWriter pw = new PrintWriter(
			...
			for (Student s : gr.getStudents()) {
				...
				pw.printf("%s,%s,%s,%d,%s%n", nvl(s.getName()), nvl(s.getLastName()), gender, s.getId(), nvl(gName));
			}
		} catch (IOException e) {
	...
	public Group loadGroupFromCSV(File file) {
		...
		try (BufferedReader br = new BufferedReader(
			...
			while ((line = br.readLine()) != null) {
				if (first) { 
					first = false;
					if (line.toLowerCase().startsWith("name,last"))
						continue;
				}
				if (line.isBlank())
					continue;
	...
	public File findFileByGroupName(String groupName, File workFolder) {
		if (groupName == null || workFolder == null || !workFolder.isDirectory())
			return null;
